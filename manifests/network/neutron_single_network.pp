...
define manila::network::neutron_single_network (
  ...
  $neutron_net_id     = undef,
  $neutron_subnet_id  = undef,
) {

  $neutron_single_plugin_name = 'manila.network.neutron.neutron_network_plugin.NeutronSingleNetworkPlugin'

  manila_config {
    "${share_backend_name}/network_api_class": value => $neutron_single_plugin_name;
    "${share_backend_name}/neutron_net_id":    value => $neutron_net_id;
    "${share_backend_name}/neutron_subnet_id": value => $neutron_subnet_id;
  }
}
